...
document.addEventListener("click", (e) => {
   ...
   if (e.target.closest(".logo")) console.log("logo 클릭");
   else if (e.target.closest("#signinForm > .sign-btn")) if (pass) inForm.submit();
   else if (e.target.closest("#signupForm > .sign-btn")) if (pass) upForm.submit();
   else e.preventDefault();
});

/// 로그인 submit
inSubmit.addEventListener("click", (event) => {
    pass = Object.values(inCheck).every(Boolean); // 딕셔너리 -> 배열로 변환 후 모든 요소가 true인지 확인하는 코드
});

/// 회원가입 submit
upSubmit.addEventListener("click", (event) => {
    pass = Object.values(upCheck).every(Boolean); // 딕셔너리 -> 배열로 변환 후 모든 요소가 true인지 확인하는 코드
});
